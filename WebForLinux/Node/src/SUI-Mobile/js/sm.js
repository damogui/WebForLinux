...
+function ($) {
    ...
    $.confirm2 = function (text, title, callbackOk, callbackCancel, canceltext, oktext) {
        if (typeof title === 'function') {
            callbackCancel = arguments[2];
            callbackOk = arguments[1];
            title = undefined;
        ...
        if (oktext != undefined && oktext != "") {
            defaults.modalButtonOk = oktext;//确定的文字
        }
        if (canceltext != undefined && canceltext != "") {
            defaults.modalButtonCancel = canceltext;//取消的文字
        }
        return $.modal({
            text: text || '',
            title: typeof title === 'undefined' ? defaults.modalTitle : title,
