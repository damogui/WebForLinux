...
LuiDropDownList.prototype = {
    ...
    init: function (param) {
        ...
        for (var k = 0; k < data.length; k++) {
            ...
            var itemHtml = '<li title=' + item[textField] + ' data-index=' + k + ' data-id=' + item[valueField] + ' data-json=\'' + JSON.stringify(data[k]).replace(/'/g, this.keychar)+'\'>' + v + '</li>';
            ulHtml += itemHtml;
        }
        ulHtml += "</ul>";
        ulHtml += "</div>";
        ...
        li.click(function () {
            ...
            var r=new RegExp(drop.keychar,"g");
            var reljson = selectedJson.replace(r, "'");
            //选中回调事件
            if (selectedCallBack) {
                selectedCallBack(warpid, selectedValue, alltitle, reljson);
            }
            dropdiv.slideUp(200);
            return false;
    ...
    getSelectedJsonValue: function () {
        if (this.param.data.length > 0) {
            var span = this.span;
            var r = new RegExp(this.keychar, "g");
            var reljson = span.attr("data-json").replace(r, "'");
            return JSON.parse(reljson);
        }
        else {
            return null;
        }
    ...
    setValue: function (value) {
        ...
        $(this.selector).attr("data-id", value);
